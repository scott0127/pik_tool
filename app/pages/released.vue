  ...
  <div class="min-h-screen pt-20 pb-24 lg:pt-24 lg:pb-8 relative">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 relative z-10">
      ...
      <div class="mb-8 text-center sm:text-left gsap-stagger">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent mb-2">
          {{ $t('released.title') }}
        </h1>
      ...
      <!-- Action Bar -->
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8 bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl p-4 rounded-2xl shadow-sm border border-white/40 dark:border-gray-700/40 gsap-stagger">
        <button
          @click="openModal()"
      ...
      <!-- Stats / Counter -->
      <div v-if="getRecordCount() > 0" class="mb-4 text-sm text-gray-500 dark:text-gray-400 font-medium gsap-stagger">
      ...
      <div v-if="filteredRecords.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div 
          ...
          :key="record.id"
          class="gsap-card bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-2xl border border-white/30 dark:border-gray-700/30 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 p-4 flex flex-col gap-3 group relative overflow-hidden"
        >
          <!-- Item Info -->
      ...
      <div v-else class="text-center py-20 text-gray-500 dark:text-gray-400 gsap-stagger">
    ...
    >
      <div v-if="showModal" class="fixed inset-0 z-[2000] flex items-center justify-center p-4">
        <div class="modal-bg absolute inset-0 bg-black/50 backdrop-blur-sm" @click="closeModal"></div>
        
        <div class="modal-panel relative bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          <!-- Modal Header -->
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto custom-scrollbar p-1">
                  ...
                  >
                    <div class="w-10 h-10 relative">
                      ...
                      <div class="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border border-white dark:border-gray-800" :class="PIKMIN_TYPE_COLORS[item.pikminType as keyof typeof PIKMIN_TYPE_COLORS]"></div>
                    </div>
